<script lang="ts">
	...
	function updateIndicator() {
		...
		// Settings is separate — check that too
		...
		indicatorStyle = `width: ${rect.width}px; height: ${rect.height}px; transform: translateX(${rect.left - containerRect.left}px) translateY(${rect.top - containerRect.top}px); opacity: 1;`;
	}
...
<nav bind:this={navContainer} class="flex items-center gap-0.5 overflow-x-auto glass-subtle rounded-xl p-1 relative">
	{#if mounted}
		<div
			class="absolute rounded-lg pointer-events-none transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]"
			style="{indicatorStyle} background: color-mix(in srgb, var(--color-primary) 12%, transparent); border: 1px solid color-mix(in srgb, var(--color-primary) 20%, transparent);"
			aria-hidden="true"
		></div>
	{/if}
	...
	{#each tabs as tab}
		...
		<a
			href={tab.href}
			data-repo-tab
			class="flex items-center gap-2 px-3.5 py-2 text-xs transition-colors duration-200 rounded-lg whitespace-nowrap relative z-10"
			style="
	...
	<a
		...
		data-repo-settings
		class="flex items-center gap-2 px-3.5 py-2 text-xs transition-colors duration-200 rounded-lg whitespace-nowrap ml-auto relative z-10"
		style="

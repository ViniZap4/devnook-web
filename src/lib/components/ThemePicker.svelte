...
<Dropdown bind:open width="min-w-56" panelClass="max-h-[70vh] overflow-y-auto">
	{#snippet trigger()}
		<span
			class="flex items-center gap-2 px-3 py-2 rounded-xl border transition-colors text-sm"
			style="border-color: var(--glass-border); color: var(--color-text-dim); background: rgba(255, 255, 255, 0.03);"
	...
	{#snippet children()}
		<!-- Mode selector -->
		<div class="flex border-b" style="border-color: var(--glass-border); background: rgba(255, 255, 255, 0.03);">
			{#each modes as m}
				<button
					onclick={() => setMode(m)}
		...
		{#if themeStore.mode !== 'light'}
			...
			{#each darkThemeOrder as name}
				...
				<button
					...
					style="color: {selected ? t.primary : 'var(--color-text-dim)'}; background: {selected ? 'rgba(255, 255, 255, 0.03)' : 'transparent'};"

...
<div class="flex flex-col gap-3">
	<div class="flex items-center justify-between flex-wrap gap-3">
		<div class="flex items-center gap-2 min-w-0">
			...
			<a href="/{repo.owner}" class="text-lg hover:underline shrink-0" style="color: var(--palette-0);">
				{repo.owner}
			</a>
			...
			<span class="text-[0.625rem] px-2 py-0.5 rounded-full border shrink-0 {repo.is_private ? 'border-yellow-500/20 text-yellow-500/60' : 'border-white/10 text-[var(--color-text)] opacity-30'}">
				{repo.is_private ? 'Private' : 'Public'}
			</span>
		</div>

		<button
			class="flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-lg border border-white/10 text-[var(--color-text)] hover:bg-white/5 transition-colors shrink-0"

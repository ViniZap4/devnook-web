<script lang="ts">
	import type { Branch } from '$lib/types/repository';
	import BranchIcon from '$lib/assets/icons/BranchIcon.svelte';
	import Dropdown from './Dropdown.svelte';

	let { branches, currentRef, onselect }: {
		branches: Branch[];
		currentRef: string;
		onselect: (branch: string) => void;
	} = $props();

	let open = $state(false);
</script>

<Dropdown bind:open align="left" width="min-w-[200px]">
	{#snippet trigger()}
		<span
			class="flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg border border-[var(--glass-border)] bg-[rgba(255,255,255,0.03)] text-[var(--color-text)] hover:bg-[rgba(255,255,255,0.06)] transition-colors"
		>
			<BranchIcon size={14} />
			<span class="font-medium">{currentRef}</span>
			<svg class="w-3 h-3 opacity-50" viewBox="0 0 12 12" fill="currentColor">
				<path d="M6 8.825a.5.5 0 0 1-.354-.146l-3.5-3.5a.5.5 0 1 1 .708-.708L6 7.618l3.146-3.147a.5.5 0 1 1 .708.708l-3.5 3.5A.5.5 0 0 1 6 8.825Z" />
			</svg>
		</span>
	{/snippet}

	{#snippet children()}
		<div class="p-2 border-b border-[var(--glass-border)]">
			<p class="text-xs text-[var(--color-text)] opacity-50 px-2">Switch branch</p>
		</div>
		<div class="max-h-60 overflow-y-auto p-1">
			{#each branches as branch}
				<button
					class="flex items-center gap-2 px-3 py-2 text-sm rounded-md w-full text-left transition-colors {branch.name === currentRef ? 'bg-[rgba(255,255,255,0.03)] font-medium' : 'hover:bg-[rgba(255,255,255,0.03)]'}"
					style="color: var(--color-text);"
					onclick={() => { onselect(branch.name); open = false; }}
				>
					<BranchIcon size={12} />
					{branch.name}
					{#if branch.is_default}
						<span class="text-xs opacity-40 ml-auto">default</span>
					{/if}
				</button>
			{/each}
		</div>
	{/snippet}
</Dropdown>

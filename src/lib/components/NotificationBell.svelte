...
<Dropdown bind:open width="w-80">
	{#snippet trigger()}
		<span
			class="relative p-2 rounded-lg transition-all duration-200 hover:bg-[rgba(255,255,255,0.03)] flex items-center justify-center"
	...
	{#snippet children()}
		<div class="max-h-96 overflow-y-auto">
			<div class="flex items-center justify-between px-4 py-3 border-b" style="border-color: var(--glass-border);">
				...
				{#if items.length > 0}
					<button
						class="text-xs px-2 py-1 rounded-lg transition-colors hover:bg-[rgba(255,255,255,0.06)]"
			...
			{:else}
				{#each items as item, i}
					<div
						...
						style="border-color: var(--glass-border); animation: fade-up 0.2s ease both; animation-delay: {i * 30}ms;"
					>
						...
						<button
							class="text-xs shrink-0 opacity-0 group-hover:opacity-100 p-1 rounded transition-all duration-200 hover:bg-[rgba(255,255,255,0.06)]"
							style="color: var(--color-text-dim);"

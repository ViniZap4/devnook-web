<script lang="ts">
	...
	async function closeMilestone(id: number) {
		try {
			await milestones.update(owner, repo, id, { state: 'closed' });
			await fetchMilestones();
		} catch {
			// ignore
		}
				...
				<div class="rounded-xl border p-5" style="border-color: var(--color-border);">
					<div class="flex items-start justify-between gap-3">
						...
						{#if milestone.state === 'open'}
							<button
								class="text-xs px-3 py-1.5 rounded-lg border transition-colors hover:bg-[var(--color-surface-hover)] shrink-0"
								style="border-color: var(--color-border); color: var(--color-text-dim);"
								onclick={() => closeMilestone(milestone.id)}
							>
								Close
							</button>
						{/if}

		...
		<form onsubmit={handleCreate} class="card p-5 flex flex-col gap-4">
			<div class="flex items-end gap-3">
				<div class="flex-1">
					...
					<input type="text" bind:value={name} placeholder="e.g. bug, enhancement, help wanted" class="w-full px-3.5 py-2.5 text-sm rounded-xl border transition-colors focus:border-[var(--color-primary)]" style="border-color: var(--glass-border); background-color: var(--color-background); color: var(--color-text);" />
				</div>
				<div>
			...
			<div>
				...
				<input type="text" bind:value={description} placeholder="Describe this label..." class="w-full px-3.5 py-2.5 text-sm rounded-xl border transition-colors focus:border-[var(--color-primary)]" style="border-color: var(--glass-border); background-color: var(--color-background); color: var(--color-text);" />
			</div>
			<div class="flex items-center gap-3">
				<div class="flex items-center gap-2">
	...
	{:else if items.length > 0}
		<div class="card overflow-hidden">
			{#each items as label, i}
				<div class="px-5 py-3.5 transition-colors hover:bg-[rgba(255,255,255,0.03)] {i > 0 ? 'border-t' : ''}" style="border-color: var(--glass-border);">
					{#if editingId === label.id}
						<!-- Edit mode -->
						<div class="flex flex-col gap-3">
							<div class="flex items-center gap-3 flex-wrap">
								<input type="text" bind:value={editName} class="px-3 py-1.5 text-sm rounded-lg border transition-colors focus:border-[var(--color-primary)] flex-1 min-w-[120px]" style="border-color: var(--glass-border); background-color: var(--color-background); color: var(--color-text);" />
								<div class="flex items-center gap-2">
							...
							<input type="text" bind:value={editDescription} placeholder="Description" class="px-3 py-1.5 text-sm rounded-lg border transition-colors focus:border-[var(--color-primary)]" style="border-color: var(--glass-border); background-color: var(--color-background); color: var(--color-text);" />
							<div class="flex items-center gap-2">

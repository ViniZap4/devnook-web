...
<div class="flex flex-col gap-6">
	...
	{#if loading}
		<div class="card overflow-hidden card-animate">
			{#each Array(4) as _, i}
				<div class="flex items-center gap-3 px-5 py-4 {i > 0 ? 'border-t' : ''}" style="border-color: var(--glass-border);">
	...
	{:else if !hasWiki || wikiPages.length === 0}
		<div class="card p-16 text-center card-animate">
			<div class="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center" style="background: var(--color-info-subtle);">
		...
		<div class="card overflow-hidden card-animate">
			{#each wikiPages as entry, i}
				...
				<a
					...
					class="flex items-center justify-between px-5 py-3.5 transition-all hover:bg-[rgba(255,255,255,0.06)] group {i > 0 ? 'border-t' : ''}"
					style="border-color: var(--glass-border);"
				>
					<div class="flex items-center gap-3">
						<div class="w-7 h-7 rounded-lg flex items-center justify-center" style="background: var(--color-info-subtle);">
						...
						{#if slug === 'Home'}
							<span class="text-[10px] px-1.5 py-0.5 rounded-md font-medium" style="background: var(--color-primary-subtle); color: var(--color-primary);">Home</span>
						{/if}
					</div>

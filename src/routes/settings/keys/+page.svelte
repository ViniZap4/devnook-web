	...
	<div class="flex flex-col gap-8">
		...
		{#if showForm}
			<form onsubmit={handleCreate} class="card p-6 flex flex-col gap-4">
				<div>
					...
					<input type="text" bind:value={name} placeholder="My Laptop" class="w-full px-3 py-2 text-sm rounded-lg border" style="border-color: var(--glass-border); background-color: #0f1629; color: var(--color-text);" />
				</div>
				<div>
					<label class="block text-xs font-medium mb-1.5" style="color: var(--color-text-dim);">Key</label>
					<textarea bind:value={publicKey} placeholder="ssh-rsa AAAA... or ssh-ed25519 AAAA..." rows={4} class="w-full px-3 py-2 text-sm rounded-lg border font-mono resize-y" style="border-color: var(--glass-border); background-color: #0f1629; color: var(--color-text);"></textarea>
				</div>
				{#if error}
		...
		{:else}
			<div class="card overflow-hidden">
				{#each keys as key, i}
					<div class="flex items-center justify-between px-4 py-3 {i > 0 ? 'border-t' : ''}" style="border-color: var(--glass-border);">
						<div>
						...
						<button class="text-xs px-3 py-1 rounded-lg border transition-colors btn-danger-subtle" style="border-color: var(--glass-border); color: var(--color-error);" onclick={() => handleDelete(key.id)}>Delete</button>
					</div>
				{/each}
			</div>
		{/if}
